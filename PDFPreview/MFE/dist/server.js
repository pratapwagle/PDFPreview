...
app.get('/uwp-test', (req, res) => {
    res.sendFile(path_1.default.join(__dirname, '../public/uwp-test.html'));
});
// Route to serve the chunked streaming test page
app.get('/chunked-test', (req, res) => {
    res.sendFile(path_1.default.join(__dirname, '../public/chunked-streaming-test.html'));
});
// Route to serve the method validation test page
app.get('/method-test', (req, res) => {
    res.sendFile(path_1.default.join(__dirname, '../public/method-validation-test.html'));
});
// Route to serve the timing test page
app.get('/timing-test', (req, res) => {
    res.sendFile(path_1.default.join(__dirname, '../public/timing-test.html'));
});
    ...
    try {
        ...
        if (path_1.default.extname(filePath).toLowerCase() !== '.pdf') {
            ...
            return res.status(400).json({
                ...
                filePath: filePath,
                fileExtension: path_1.default.extname(filePath)
            });
        }
...
app.get('/api/uwp/config', (req, res) => {
    try {
        const configPath = path_1.default.join(__dirname, '../uwp-config.json');
...
app.get('/api/debug/path/:path(*)', (req, res) => {
    try {
        ...
        console.log('File exists:', fs_1.default.existsSync(decodedPath));
        res.json({
            rawPath: rawPath,
            decodedPath: decodedPath,
            exists: fs_1.default.existsSync(decodedPath),
            isFile: fs_1.default.existsSync(decodedPath) ? fs_1.default.statSync(decodedPath).isFile() : false,
            extension: path_1.default.extname(decodedPath),
            timestamp: new Date().toISOString()
        });
    }
    catch (error) {
